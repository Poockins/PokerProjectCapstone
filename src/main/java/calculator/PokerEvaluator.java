 ...
 */
 public class PokerEvaluator {
   ...
   public boolean hasPair(ArrayList<Cards> userCards, ArrayList<Cards> communityCards) {
     int numOfPairs, numOfPairs1;
     numOfPairs = numOfPairs1 = 0;
     ...
     if (userCards.get(0).getRank().getRankValue() == userCards.get(1).getRank().getRankValue()) {
       numOfPairs++;// since the users card match between each other
   ...
   public boolean hasfullHouse(ArrayList<Cards> userCards, ArrayList<Cards> communityCards) {
     return (hasThreeOfKind(userCards, communityCards) && hasPair(userCards, communityCards));
   ...
   public boolean hasTwoPairs(ArrayList<Cards> userCards, ArrayList<Cards> communityCards) {
     ...
     if (userCards.get(0).getRank().getRankValue() == userCards.get(1).getRank().getRankValue())
     {
         ...
         if(sameKind ==3)//four cards with the same rank
       return true;// impossible to have two pairs if your only two cards are same
         else
         	return false;
     }
   ...
   public boolean hasRoyalFlush(ArrayList<Cards> userCards, ArrayList<Cards> communityCards) {
 	 ...
 	 //boolean array.
 	  boolean[] boolRoyalArray = new boolean[5];
     ...
     boolRoyalArray[0] = aceCard;
     boolRoyalArray[1] = kingCard;
     boolRoyalArray[2] = queenCard;
     boolRoyalArray[3] = jackCard;
     boolRoyalArray[4] = tenCard;
     ...
     for (Cards card : allCards) {
       if (card.getRank().getRankValue() == 10) {
     	  boolRoyalArray[4] = true;
         royalCards.add(card);// adding the card
       } else if (card.getRank().getRankValue() == 11) {
     	  boolRoyalArray[3] = true;
         royalCards.add(card);// adding the card
       } else if (card.getRank().getRankValue() == 12) {
     	  boolRoyalArray[2] = true;
         royalCards.add(card);// adding the card
       } else if (card.getRank().getRankValue() == 13) {
     	  boolRoyalArray[1] = true;
         royalCards.add(card);// adding the card
       } else if (card.getRank().getRankValue() == 14) {
     	  boolRoyalArray[0] = true;
         royalCards.add(card);// adding the card
       }

     }

     if ( boolRoyalArray[4] &&  boolRoyalArray[3] &&  boolRoyalArray[2] &&  boolRoyalArray[1] &&  boolRoyalArray[0]) {
